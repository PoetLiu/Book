/*
 *
 * Authored by Liu Peng on: Tue Nov  4 09:16:35 CST 2014
 *
 * @desc: a tiny calculator that uses dc to do its work
 *	  demonstrates bidirectional pipes
 *	  input looks like number op number which
 *	  tinybc converts into number \n number \n op \n p
 *	  and passes result bakc to stdout
 *        +----------+		      +----------+
 * stdin >0	     >== pipetodc ====>		 |
 *	  |   tinybc |		      |    dc -  |
 * stdout<0          <== pipefrom ====<          |
 *	  +----------+                +----------+
 *
 * @func: a. get two pipes
 *	  b. fork(get another process)
 *	  c. in the dc-to-be process, connect stdin
 *	     and out to pipes then execl dc
 *	  d. int the tinybc-process, no plumbing to do 
 *	     just talk to human via normal i/o
 *	     and send stuff via pipe
 *	  e. then close pipe and dc dies
 * @noet: does not handle multiline answers
**/
...
int main(void)
{
	...
	if (pid == 0)				/* child is dc */
		...
		be_bc(todc, fromdc);		/* parent if ui */
 ...
 * Funcion	:
 *
 * Author	: set up stdin and stdout, then execl dc
 *
 * arguments	:
 * return	:
**/
...
/*
 * Funcion	: read from stdin and convert into RPN, send down 
 *		  pipe then read from other pipe and print to user
 *		  uses fdopen() to convert a file descriptor to a 
 *		  stream
 * Author	: 
 *
 * arguments	:
...
void be_bc(int todc[2], int fromdc[2])
{
	...
	fpin	= fdopen(fromdc[0], "r"); /* riptors to sreams */
