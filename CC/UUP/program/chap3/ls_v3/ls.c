/*
   ...
   * 相比上一个版本, 修正了如下bug:
   * 1. 增加 -l -a 等可选参数, 可选择列出所有文件或一般文件以及普通格式或者长格式
   * 此版本的BUG
   * 1. 不加-l 时， 一般模式输出没有颜色， 程序流程、架构需要调整
 */
...
{
	int i, j = 0, ws_col, temp, lines = 1, cols, col_mw, col_tw, dot_count = 0;
	nlink_t max_links = 0;
	off_t max_size = 0;
	int max_uname = 0;
	int max_gname = 0;
	...
	for (i = 0; i < n; i++) {
		...
		if (!long_format) {
			...
			tmp_name[j] = (char *) malloc(fnames_l[j]);
	...
	if (!long_format) {
		...
		if (temp > -1) {
			...
			pf->col_w	= fnames_l;
			for (i = 0; i < n; i++)
				printf("%-*s  ", pf->col_w[i], tmp_name[i]);
			return;
		} 
		...
		while(1) {
			...
			if (col_tw <= ws_col) {
				for (i = 0; i < lines; i++) {
					for (j = 0; j < cols && cols*i + j <= n; j++) {
						if (j*lines + i < n) {
							printf("%-*s  ", pf->col_w[j], tmp_name[j*lines + i]);
							free(tmp_name[j*lines + i]);
						}
					}
					if (i < lines - 1)
						printf("\n");
...
void do_ls(char dirname[])
{
	...
	if (n < 0)
		...
		for(i = 0; i < n; i++) {
			if (!long_format)
				break;
			if (namelist[i]->d_name[0] == '.' && !print_all)
				continue;
